...
public final class Items {
    ...
    public static Material material(String s) {
        ...
        return switch (s.toLowerCase(Locale.ROOT)) {
            case "leather" -> Material.LEATHER_CHESTPLATE; case "chainmail" -> Material.CHAINMAIL_CHESTPLATE;
            case "iron" -> Material.IRON_CHESTPLATE; case "gold" -> Material.GOLDEN_CHESTPLATE;
            case "diamond" -> Material.DIAMOND_CHESTPLATE; case "netherite" -> Material.NETHERITE_CHESTPLATE;
            default -> null;
        };
    }
    ...
    public static ItemStack armor(Material mat, String piece) {
        ...
        String suffix = switch (piece.toLowerCase(Locale.ROOT)) {
            case "head", "helmet" -> "HELMET"; case "chest", "chestplate" -> "CHESTPLATE";
            case "legs", "leggings" -> "LEGGINGS"; case "feet", "boots" -> "BOOTS"; default -> "BOOTS";
        };
        String n = mat.name().replace("_CHESTPLATE", "") + "_" + suffix;
        try { return new ItemStack(Material.valueOf(n)); } catch (Exception e) { return new ItemStack(Material.AIR); }
    }

    public static void enchant(ItemStack item, Enchantment ench, int level) {
        if (item == null || item.getType().isAir() || ench == null || level < 1 || level > 255) return;
    ...
    public static Enchantment enchantment(String name) {
        ...
        for (Enchantment e : Enchantment.values()) {
            if (e.getKey().getKey().equalsIgnoreCase(n) || e.getKey().toString().equalsIgnoreCase(n)) return e;
        }
        return null;
    }
    ...
    public static int level(String mode, String level) {
        ...
        try { int n = Integer.parseInt(level); return n >= 1 && n <= 10 ? n : -1; } catch (Exception e) { return -1; }
    }
