...
public final class UopCommandCompat implements CommandExecutor, TabCompleter {
    ...
    public UopCommandCompat(UopRecordingToolsPlugin delegatePlugin) { this.plugin = delegatePlugin; this.delegate = new UopCommand(delegatePlugin); }
    ...
    private List<String> armorTab(String[] a) {
        ...
        String action=a[1].toLowerCase(Locale.ROOT);
        switch(action){
            case "set": if(a.length==3)return players(); if(a.length==4)return ARMOR_MATERIALS; if(a.length==5)return ARMOR_MODES; if(a.length==6&&a[4].equalsIgnoreCase("enchanted"))return levels10(); if(a.length==6)return BOOLEAN_VALUES; if(a.length==7&&a[4].equalsIgnoreCase("enchanted"))return BOOLEAN_VALUES; if(a.length>=7)return WEAPONS; return null;
            case "save": if(a.length==4)return players(); return null;
            case "list": return null;
            case "delete": if(a.length==3)return loadouts(); return null;
            case "give": if(a.length==3)return players(); if(a.length==4)return List.of("saved","direct"); if(a.length==5&&a[3].equalsIgnoreCase("saved"))return loadouts(); if(a.length==5&&a[3].equalsIgnoreCase("direct"))return ARMOR_MATERIALS; if(a.length==6&&a[3].equalsIgnoreCase("direct"))return ARMOR_MODES; if(a.length==7&&a[3].equalsIgnoreCase("direct")&&a[5].equalsIgnoreCase("enchanted"))return levels10(); if(a.length==7&&a[3].equalsIgnoreCase("direct"))return BOOLEAN_VALUES; if(a.length==8&&a[3].equalsIgnoreCase("direct")&&a[5].equalsIgnoreCase("enchanted"))return BOOLEAN_VALUES; if(a.length>=8&&a[3].equalsIgnoreCase("direct"))return WEAPONS; return null;
            case "weapon": if(a.length==3)return List.of("set"); if(a.length==4&&a[2].equalsIgnoreCase("set"))return players(); if(a.length==5&&a[2].equalsIgnoreCase("set"))return ARMOR_MODES; if(a.length==6&&a[2].equalsIgnoreCase("set")&&a[4].equalsIgnoreCase("enchanted"))return levels10(); if(a.length==6&&a[2].equalsIgnoreCase("set"))return WEAPONS; if(a.length>=7&&a[2].equalsIgnoreCase("set"))return WEAPONS; return null;
            default:return null;
        }
    }
    ...
    private List<String> invTab(String[] a) {
        ...
        String action=a[1].toLowerCase(Locale.ROOT);
        if(action.equals("record")){if(a.length==3)return List.of("start","stop");if(a.length==4&&(a[2].equalsIgnoreCase("start")||a[2].equalsIgnoreCase("stop")))return players();return null;}
        if(action.equals("view")||action.equals("backups")||action.equals("lock")||action.equals("unlock")||action.equals("clear")){if(a.length==3)return players();return null;}
        if(action.equals("set")){if(a.length==3)return players();if(a.length==4)return slots41();if(a.length==5)return itemNames();if(a.length==6)return amountValues();return null;}
        if(action.equals("remove")){if(a.length==3)return players();if(a.length==4)return slots41();return null;}
        if(action.equals("give")||action.equals("take")){if(a.length==3)return players();if(a.length==4)return itemNames();if(a.length==5)return amountValues();return null;}
        if(action.equals("swap")){if(a.length==3)return players();if(a.length==4)return slots41();if(a.length==5)return players();if(a.length==6)return slots41();return null;}
        if(action.equals("backup")||action.equals("restore")||action.equals("backup-delete")){if(a.length==3)return players();if(a.length==4)return backupNames(a[2]);return null;}
    ...
    private List<String> slots41(){List<String> out=new ArrayList<>(41);for(int i=0;i<=40;i++)out.add(String.valueOf(i));return out;}
    private List<String> amountValues(){List<String> out=new ArrayList<>(99);for(int i=1;i<=99;i++)out.add(String.valueOf(i));return out;}
    private List<String> itemNames(){List<String> out=new ArrayList<>();for(Material material:Material.values())if(!material.isAir()&&material.isItem())out.add(material.getKey().getKey());return out;}
    private List<String> enchantments(){List<String> out=new ArrayList<>();for(Enchantment enchantment:Enchantment.values())out.add(enchantment.getKey().getKey());return out;}
